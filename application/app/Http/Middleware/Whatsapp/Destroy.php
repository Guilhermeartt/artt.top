...
class Destroy {
     ...
     *   <input type="checkbox" name="ids[{{ $whatsapp->whatsapp_id }}]"
     *
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric($request->route('whatsapp'))) {
            $ids[$request->route('whatsapp')] = 'on';
            request()->merge([
                'ids' => $ids,
            ]);
        }

        //loop through each whatsapp and check permissions
        if (is_array(request('ids'))) {
            ...
            foreach (request('ids') as $id => $value) {
                ...
                if ($value == 'on') {
                    //validate
                    if (!$whatsapp = \App\Models\Timer::Where('timer_id', $id)->first()) {
                ...
                if (auth()->user()->is_team) {
                    if (auth()->user()->role->role_whatsapp >= 3) {
                        ...
                        if (auth()->user()->role->role_whatsapp_scope == 'own') {
                            if ($whatsapp->timer_creatorid != auth()->id()) {
            ...
            Log::error("no items were sent with this request", ['process' => '[permissions][whatsapp][change-category]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'whatsapp id' => $whatsapp_id ?? '']);
            abort(409);
        }
