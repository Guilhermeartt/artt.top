...
class Destroy {
     ...
     *   <input type="checkbox" name="ids[{{ $listmessage->listmessage_id }}]"
     *
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric($request->route('listmessage'))) {
            $ids[$request->route('listmessage')] = 'on';
            request()->merge([
                'ids' => $ids,
            ]);
        }

        //loop through each listmessage and check permissions
        if (is_array(request('ids'))) {
            ...
            foreach (request('ids') as $id => $value) {
                ...
                if ($value == 'on') {
                    //validate
                    if (!$listmessage = \App\Models\Timer::Where('timer_id', $id)->first()) {
                ...
                if (auth()->user()->is_team) {
                    if (auth()->user()->role->role_listmessages >= 3) {
                        ...
                        if (auth()->user()->role->role_listmessages_scope == 'own') {
                            if ($listmessage->timer_creatorid != auth()->id()) {
