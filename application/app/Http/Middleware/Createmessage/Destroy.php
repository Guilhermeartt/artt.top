...
class Destroy {
     ...
     *   <input type="checkbox" name="ids[{{ $createmessage->createmessage_id }}]"
     *
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric($request->route('createmessage'))) {
            $ids[$request->route('createmessage')] = 'on';
            request()->merge([
                'ids' => $ids,
            ]);
        }

        //loop through each createmessage and check permissions
        if (is_array(request('ids'))) {
            ...
            foreach (request('ids') as $id => $value) {
                ...
                if ($value == 'on') {
                    //validate
                    if (!$createmessage = \App\Models\Timer::Where('timer_id', $id)->first()) {
            ...
            Log::error("no items were sent with this request", ['process' => '[permissions][createmessages][change-category]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'createmessage id' => $createmessage_id ?? '']);
            abort(409);
        }
